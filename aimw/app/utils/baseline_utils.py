...
from aimw.app.services.batch.batch_qgen_agen_service import QAGenerator


def apply_generator(
    cct_saar_generator: QAGenerator,
    r_dir: str,
    w_dir: str,
    start_index: int = 0,
    end_index: int = -1,
    ...
    for i in range(start_index, end_index):
        ...
        with open(f"{r_dir}base_corpus_cln_split_{str(i)}.json", encoding="utf-8") as f:
            data = json.load(f)
        ...
        with open(f"{w_dir}baseline_success_{str(i)}.json", "w", encoding="utf-8") as f:
            json.dump(cct_saar_generated, f)

        if len(cct_saar_failed) > 0:  # save cct_saar_failed
            with open(
                f"{w_dir}baseline_failed_{str(i)}.json",
                "w",
                encoding="utf-8",
            ) as f:
...
def reprocess_cct_saar(cct_saar_generator: QAGenerator, dir: str, sleep_time: int = 0):
    """
    ...
    # filtered_files = [f for f in os.listdir(w_dir) if os.path.isfile(os.path.join(w_dir, f)) and pattern.match(f)]
    file_numbers = sorted(
        [
            int(match.group(1))
